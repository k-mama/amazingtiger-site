...
functions/api/
  consultation.ts           Validates a consultation inquiry and relays it by email via Resend
  chat.ts                   Skeleton — will power the consultation chatbot
  order-notify.ts           Best-effort admin email alert for new private order requests (Resend)
...
**Admin visibility:** `/[locale]/admin` has a live Orders section (`components/AdminOrdersList.tsx`) below the consultations list — recent order requests with created date, customer, email, phone, country, region, status, subtotal, and item count, with an expandable per-order panel showing line items plus the full billing address, shipping address (or "same as billing"), and order notes. Each row has a status dropdown (New / Contacted / Completed / Cancelled) that updates `orders.status` directly, backed by the existing `"Admins can manage orders"` RLS policy — no new policy was needed. Still no delete, no refund/shipping/inventory tooling, no statistics.

**Order-request notifications:** submitting checkout also fires a best-effort `POST /api/order-notify` (`functions/api/order-notify.ts`) after the Supabase insert succeeds, sending an admin email via Resend with the order reference, customer contact info, and item summary — reusing the same `RESEND_API_KEY` / `CONSULTATION_TO_EMAIL` / `CONSULTATION_FROM_EMAIL` already configured for the consultation form. A failed notification never blocks or changes the customer's success screen; the Supabase row is the source of truth either way.

Payment, real checkout, marking anything as paid, and moving product management into Supabase are all explicitly out of scope for this pass.
